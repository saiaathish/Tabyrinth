...
|---|---|---|---|---|---|
| First-run onboarding | Four short stages labelled INTRO, TRAIL, DETOUR, PORTAL | Teaches the core vocabulary before the Side Panel | Completion is stored under `tabyrinth.onboarding` in `chrome.storage.local`; the UI reads it before rendering the main surface | `PROVEN_AUTOMATED` — component coverage; live extension blocked | Yes, briefly |
...
| Replay introduction | `Replay introduction` | Reopens onboarding for an existing user | The onboarding hook resets completion and renders the same staged component | `PROVEN_AUTOMATED`; live not tested | Optional |
