...
- Current automated gate: PASS — 23 test files, 151 tests; typecheck, lint, production build, extension validator, and `git diff --check` all pass. Build emits Side Panel, Portal, Popup, Room, and service-worker bundles. Explicit current-tab tracking covers openerless tabs without weakening fail-closed automatic capture.
...
- Chrome extension manager is currently not claimable through the connected browser surface. Untouched-profile Fold/Unseal, Arcade tab lifecycle, and fresh copied-profile replay after the compatibility recovery remain `UNVERIFIED`.
