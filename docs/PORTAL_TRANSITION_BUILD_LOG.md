...
- Earlier 102/103/104/105/137/140-test entries above are historical checkpoints; the current authoritative total is recorded at the latest release checkpoint below.
...
## Historical release gate — 2026-08-12
...
- Current automated gate: PASS — 23 test files, 158 tests; typecheck, lint, production build, extension validator, and `git diff --check` all pass. Build emits Side Panel, Portal, Popup, Room, and service-worker bundles. Explicit current-tab tracking covers openerless tabs without weakening fail-closed automatic capture.
- Generated-extension E2E: PARTIAL — Chrome 151 launched in an isolated profile and exposed the unpacked MV3 service worker, but test-created extension pages loaded as `chrome-error://chromewebdata`; no extension lifecycle result is claimed from that run. The report is preserved at `artifacts/e2e/report.json`.
- Direct browser check: the current user Chrome remained on Gmail, so no untouched-profile Tabyrinth behavior is claimed. Fold/Unseal, second-tab lifecycle, and fresh copied-profile replay remain `UNVERIFIED`.
- AO CLI remains unavailable; the existing AO limitation and isolated-worker evidence are not presented as AO board/PR/CI proof.
