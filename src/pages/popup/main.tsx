...
function App() {
  ...
  return <main className="popup-shell" aria-busy={busy}>
    ...
    {run && <>
      ...
      {confirmReset && <div className="confirm" role="dialog" aria-modal="true" aria-labelledby="reset-title"><h2 id="reset-title">Close this run?</h2><p>Only TABYRINTH-owned tabs will close.</p><button onClick={() => void reset}>Reset run</button><button className="quiet" onClick={() => setConfirmReset(false)}>Keep playing</button></div>}
    </>}
    {notice && view !== "error" && <p className="notice" role="status">{notice}</p>}
  </main>;
}

const css = `:root{color-scheme:dark;font:14px/1.45 system-ui,sans-serif}*{box-sizing:border-box}body{margin:0;background:#10131a;color:#e8edf7}.popup-shell{width:340px;min-height:420px;padding:24px;background:radial-gradient(circle at 90% 0,#273047,#10131a 48%)}header{margin-bottom:24px}.eyebrow,.step{font-size:10px;letter-spacing:.16em;color:#8d9ab4;font-weight:700}h1{font:800 31px/1 Georgia,serif;letter-spacing:.08em;margin:9px 0 8px;color:#fff}h2{font:700 22px/1.1 Georgia,serif;margin:10px 0}p{color:#acb5c7;margin:8px 0 18px}.card{border:1px solid #3b465c;border-radius:12px;padding:20px;background:#191f2b;box-shadow:0 12px 35px #080a0f88}.sigil{font-size:28px;color:#e9bb69}.card button,.confirm button{width:100%;border:0;border-radius:7px;padding:11px 14px;background:#e9bb69;color:#17130d;font-weight:800;cursor:pointer}.card button:focus-visible,.confirm button:focus-visible,.reset-link:focus-visible{outline:3px solid #a9d7ff;outline-offset:3px}.card small{display:block;text-align:center;color:#7f8aa0;margin-top:12px}.tab-demo{display:flex;gap:5px;overflow:hidden;margin:18px 0}.tab-demo span{flex:1;min-width:0;padding:9px 4px;border:1px solid #59677f;border-radius:5px;text-align:center;font-size:10px;color:#d8e0ef}.danger{border-color:#a65f5f}.danger strong{color:#ffb4a8}.danger button{margin-top:7px}.quiet{background:transparent!important;color:#b7c2d7!important;border:1px solid #4e5a71!important}.reset-link{display:block;margin:18px auto 0;background:transparent;border:0;color:#8995ab;text-decoration:underline;cursor:pointer}.notice,.status{text-align:center;color:#ffc9a2;font-size:12px}.confirm{position:fixed;inset:80px 18px auto;background:#252d3d;border:1px solid #64718a;border-radius:10px;padding:18px;box-shadow:0 15px 40px #000}.confirm button{margin-top:8px}dl{display:flex;gap:20px;margin:18px 0}dt{font-size:11px;color:#8995ab}dd{margin:2px 0;font-size:20px;font-weight:700}@media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important;transition:none!important}}`;
