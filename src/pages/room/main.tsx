...
export function RoomPage({ state, error = false, onIntent }: { state: GameState | null; error?: boolean; onIntent?: (intent: Intent, roomId: string) => void }) {
  ...
  const recoveryLink = <a className="state-link" style={{ display: "inline-block", marginTop: 12, color: "#f3c969", fontWeight: 700, textDecoration: "underline", textUnderlineOffset: 4 }} href={chrome.runtime.getURL("popup.html")}>Open TABYRINTH to start a run</a>;
