...
export function createChromeMock() {
  const listeners={ moved:event<(id:number)=>void>(), removed:event<(id:number)=>void>(), activated:event<(activeInfo:{tabId:number;windowId:number})=>void>(), detached:event<(id:number,info:{oldWindowId:number,oldPosition:number})=>void>(), attached:event<(id:number,info:{newWindowId:number,newPosition:number})=>void>(), updated:event<(id:number,changeInfo:{groupId?:number},tab:chrome.tabs.Tab)=>void>(), createdNavigationTarget:event<(details:{sourceTabId:number;sourceFrameId:number;tabId:number;url:string})=>void>() };
  ...
  const chromeMock = { storage: { local: { get: async (key:string) => ({[key]: session[key]}), set: async (value:Record<string,unknown>) => Object.assign(session,value), remove: async (key:string) => delete session[key] }, session: { get: async (key:string) => ({[key]: session[key]}), set: async (value:Record<string,unknown>) => Object.assign(session,value), remove: async (key:string) => delete session[key] } },
    ...
    webNavigation: { onCreatedNavigationTarget: listeners.createdNavigationTarget }, tabGroups: { update: async () => undefined } };
