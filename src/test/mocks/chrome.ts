...
export function createChromeMock() {
  const listeners={ moved:event<(id:number)=>void>(), removed:event<(id:number)=>void>(), detached:event<(id:number,info:{oldWindowId:number,oldPosition:number})=>void>(), attached:event<(id:number,info:{newWindowId:number,newPosition:number})=>void>(), updated:event<(id:number,changeInfo:{groupId?:number},tab:chrome.tabs.Tab)=>void>() };
