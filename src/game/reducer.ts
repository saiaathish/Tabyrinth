...
export function reduceGame(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    ...
    case "RESTORE_HEALTH": { const room = getCurrentRoom(state); if (!room || room.kind !== "sanctum" || room.completed || state.player.hp === state.player.maxHp) return state; return completeRoom(changed(state, { player: { ...state.player, hp: state.player.maxHp } }), room); }
